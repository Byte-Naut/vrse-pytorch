"""High-dimensional support and C-MAPSS data-contract tests.
...
def test_cmapss_role_sizes_consume_non_discovery_units():
...
def test_cmapss_normalization_uses_only_frozen_discovery_units():
...
def test_cmapss_normalization_fails_when_floor_hides_outside_shift():
...
def test_highdim_model_lifecycle_smoke():
    ...
    review = model.review_mask(torch.cat([x_id[:5], x_new[:5]], dim=0))
    assert review.dtype == torch.bool
    assert review.shape == (10,)
