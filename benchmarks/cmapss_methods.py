"""Shared model construction and five-method C-MAPSS experiment bundle."""
...
from benchmarks.cmapss_fd002 import CMapssBatch, CMapssSplit
...
def train_baseline(split: CMapssSplit) -> RULBaseline:
    set_seed(split.seed)
...
def build_seed_template(split: CMapssSplit) -> Tuple[VRSEModel, dict]:
...
def _prequential_online_trace(template: VRSEModel, batch: CMapssBatch) -> Tuple[list, object]:
...
def run_cmapss_bundle(template: VRSEModel, split: CMapssSplit, stream: str) -> Dict[str, dict]:
...
def seed_preconditions(template: VRSEModel, split: CMapssSplit) -> dict:
    with torch.no_grad():
