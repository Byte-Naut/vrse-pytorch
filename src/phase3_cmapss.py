"""C-MAPSS FD002 data contract for VRSE Phase 3.
...
class Phase3Batch:
    ...
    def with_target(self, y: torch.Tensor) -> "Phase3Batch":
        return Phase3Batch(self.x, y, self.unit, self.cycle)
...
class Phase3Split:
    ...
    id_fit: Phase3Batch
    id_calibration: Phase3Batch
    id_guard: Phase3Batch
    shadow_observe: Phase3Batch
    promotion_validation: Phase3Batch
    post_new: Phase3Batch
    post_unknown: Phase3Batch
    ...
    def stream_targets(self, stream: str) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        raise ValueError(f"Unknown Phase-3 stream: {stream}")
...
def download_zip(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "vrse-phase3/0.1"})
...
def _role_units(seed: int) -> Dict[str, Tuple[int, ...]]:
    ...
    for name, size in ROLE_SIZES.items():
        ...
        start += size
    if start != 240:
        raise AssertionError("Phase-3 role sizes must consume exactly 240 non-discovery units.")
    return roles
...
def _batch(data: CMapssFD002, idx: np.ndarray,
           mean: np.ndarray, std: np.ndarray) -> Phase3Batch:
    if idx.size == 0:
        raise ValueError("A Phase-3 role/regime selection produced an empty batch.")
...
def make_phase3_split(data: CMapssFD002, definition: RegimeDefinition,
                      seed: int) -> Phase3Split:
...
def load_prepared(npz_path: Path) -> Tuple[CMapssFD002, RegimeDefinition]:
    ...
    if "protocol_revision" not in blob.files:
        raise RuntimeError(
            "Prepared data predates the Phase-3B normalization amendment; rerun "
            "experiments.phase3_prepare_data."
        )
