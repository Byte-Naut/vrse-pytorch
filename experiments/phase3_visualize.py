"""Generate the three pre-registered Phase-3 figures after the verdict."""
...
from experiments.phase3_common import CHECKPOINTS, MATRIX_PKL, PREPARED, RESULTS
from src.phase3_cmapss import PROTOCOL_REVISION, load_prepared, make_phase3_split
from src.phase3_methods import METHODS
...
def _load_current_checkpoint(path):
    ...
    if checkpoint.get("protocol_revision") != PROTOCOL_REVISION:
        raise RuntimeError("Checkpoint belongs to an older Phase-3 protocol.")
    return checkpoint
...
def embedding_figure() -> None:
    ...
    ax.set_title("Phase 3 frozen PhiSN space and authorization mask")
    ax.set_xlabel("PCA 1")
    ax.set_ylabel("PCA 2")
    ax.legend(fontsize=8, ncol=2)
    fig.tight_layout()
    fig.savefig(RESULTS / "phase3_embedding.png", dpi=180)
    plt.close(fig)
...
def behavior_figure(matrix: dict) -> None:
    ...
    fig.tight_layout()
    fig.savefig(RESULTS / "phase3_stream_behavior.png", dpi=180)
    plt.close(fig)
...
def safety_plasticity_figure(matrix: dict) -> None:
    ...
    ax.set_title("Phase 3 safety-plasticity plane")
    ax.legend()
    fig.tight_layout()
    fig.savefig(RESULTS / "phase3_safety_plasticity.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    print(RESULTS / "phase3_embedding.png")
    print(RESULTS / "phase3_stream_behavior.png")
    print(RESULTS / "phase3_safety_plasticity.png")
