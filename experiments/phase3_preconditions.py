"""Phase 3 / T3: run P0--P3 and freeze one shared checkpoint per seed."""
...
from src.phase3_methods import build_seed_template, seed_preconditions
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-phase2-gates", action="store_true",
        help="Debug only: skip pytest and the two Stage-4C regression commands.",
    )
    args = parser.parse_args()
    if not PREPARED.exists():
        raise FileNotFoundError(f"Run phase3_prepare_data.py first: missing {PREPARED}")

    gates = []
    if not args.skip_phase2_gates:
        gates = [
            _run_gate([sys.executable, "-m", "pytest", "tests", "-q"]),
            _run_gate([sys.executable, "-m", "experiments.exp_stage4c_vrse_regression"]),
            _run_gate([sys.executable, "-m", "experiments.exp_stage4c_vrse_gp_equivalence"]),
        ]
    ...
    payload = {
        ...
        "normalization_audit": scale_audit,
        "verdict": verdict,
        "debug_skipped_phase2_gates": bool(args.skip_phase2_gates),
        "aggregate": aggregate,
        "phase2_gates": gates,
        "seeds": rows,
    }
